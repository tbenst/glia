...
def pure_retina_spikes(units):
    """Take a Dict(unit_id,experiment) or
    Dict(unit_id,List[experiment]) and
    return Experiment[response] = Dict(unit_id,spike_train)
    
    Swaps between the outer wrapping by unit_id, for easy parallelism,
    to units contained in each experiment. I'm so sorry for how
    awful this is to read. Need better thought out types.
    Or a type system for that matter."""
#     raise(DontDoThis)
    value = deepcopy(glia.get_unit(units)[1])
    if type(value)==dict and 'stimulus' in value:
        new = value['stimulus']
        new['units'] = glia.f_map(lambda e: e['spikes'])(units)
        return new
    
    elif type(value)==dict and type(get_a_value(value))==list and \
        'stimulus' in get_a_value(value)[0]:
            new = {}
            for unit_id, dictionary in units.items():
                for k, listE in dictionary.items():
                    # new is Dict(key: List[experiment])
                    if not k in new:
                        new_list = glia.f_map(lambda x: x['stimulus'])(listE)
                        for i in range(len(new_list)):
                            new_list[i]['units'] = {}
                        new[k] = new_list
                        
                    for i,e in enumerate(listE):
                        new[k][i]["units"][unit_id] = e['spikes']
            return new
    
    elif type(value)==list and type(value[0])==dict \
                             and 'stimulus' in value[0]:
        # List[Experiment]
        stimuli = glia.f_map(lambda x: x['stimulus'])(value)

        for unit_id, experiments in units.items():
            for i, e in enumerate(experiments):
                assert stimuli[i]['stimulusIndex']==e['stimulus']['stimulusIndex']
                if not 'units' in stimuli[i]:
                    stimuli[i]["units"] = {}
                stimuli[i]["units"][unit_id] = e['spikes']
        return stimuli
    
    elif type(value)==dict and type(get_a_value(value))==dict and \
            'stimulus' in get_a_value(get_a_value(value))[0]:
            new = {}
            for unit_id, d in units.items():
                for outer_key, inner_d in d.items():
                    if not outer_key in new:
                        new[outer_key] = {}
                        
                    for inner_key, es in inner_d.items():
                        if not inner_key in new[outer_key]:
                            new[outer_key][inner_key] = \
                                glia.f_map(lambda x: x['stimulus'])(es)
                        
                        for i,e in enumerate(es):
                            # I'm so sorry. I promise to switch to a typed language soon
                            # and rethink how to program ;)
                            try:
                                new[outer_key][inner_key][i]['units'][unit_id] = e['spikes']
                            except:
                                new[outer_key][inner_key][i]['units'] = {}
                                new[outer_key][inner_key][i]['units'][unit_id] = e['spikes']
            return new
                                
    else:
        raise(ValueError, 'bad input to pure_retina_spikes')
